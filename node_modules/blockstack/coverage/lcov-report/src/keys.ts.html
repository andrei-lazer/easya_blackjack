...
<html lang="en">
<head>
    <title>Code coverage report for src/keys.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">src</a> keys.ts
    </h1>
    <div class='clearfix'>
...
export function getEntropy(arg: number): Buffer {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!arg) {
<span class="cstat-no" title="statement not covered" >    arg = 32</span>
  }
...
export function publicKeyToAddress(publicKey: string | Buffer) {
  const publicKeyBuffer = Buffer.isBuffer(publicKey) ? <span class="branch-0 cbranch-no" title="branch not covered" >publicKey </span>: Buffer.from(publicKey, 'hex')
...
export function getPublicKeyFromPrivate(privateKey: string | Buffer) {
  const privateKeyBuffer = Buffer.isBuffer(privateKey) ? <span class="branch-0 cbranch-no" title="branch not covered" >privateKey </span>: Buffer.from(privateKey, 'hex')
...
&nbsp;
  if (skHex.length === 66) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (skHex.slice(64) !== '01') {
<span class="cstat-no" title="statement not covered" >      throw new Error('Improperly formatted private-key hex string. 66-length hex usually '</span>
  ...
  } else <span class="missing-if-branch" title="if path not taken" >I</span>if (skHex.length === 64) {
<span class="cstat-no" title="statement not covered" >    ecPairOptions.compressed = false</span>
<span class="cstat-no" title="statement not covered" >    return ECPair.fromPrivateKey(Buffer.from(skHex, 'hex'), ecPairOptions)</span>
  } else {
...
export function ecPairToHexString(secretKey: ECPair.ECPairInterface) {
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (secretKey.compressed) {
    return `${ecPointHex}01`
  } else {
<span class="cstat-no" title="statement not covered" >    return ecPointHex</span>
  }
}
&nbsp;
/**
