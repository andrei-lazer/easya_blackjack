...
<html lang="en">
<head>
    <title>Code coverage report for src/storage/hub.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">src/storage</a> hub.ts
    </h1>
    <div class='clearfix'>
...
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (!dangerouslyIgnoreEtag) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (newFile) {
      headers['If-None-Match'] = '*'
    } else <span class="cstat-no" title="statement not covered" >if (etag) {</span>
<span class="cstat-no" title="statement not covered" >      headers['If-Match'] = etag</span>
    }
  }
&nbsp;
 ...
 * @ignore
 */
function <span class="fstat-no" title="function not covered" >makeLegacyAuthToken(</span>challengeText: string, signerKeyHex: string): string {
  // only sign specific legacy auth challenges.
  let parsedChallenge
<span class="cstat-no" title="statement not covered" >  try {</span>
<span class="cstat-no" title="statement not covered" >    parsedChallenge = JSON.parse(challengeText)</span>
  } catch (err) {
<span class="cstat-no" title="statement not covered" >    throw new Error('Failed in parsing legacy challenge text from the gaia hub.')</span>
  }
<span class="cstat-no" title="statement not covered" >  if (parsedChallenge[0] === 'gaiahub'</span>
      &amp;&amp; parsedChallenge[3] === 'blockstack_storage_please_sign') {
    const signer = <span class="cstat-no" title="statement not covered" >hexStringToECPair(signerKeyHex</span>
                                     + (signerKeyHex.length === 64 ? '01' : ''))
    const digest = <span class="cstat-no" title="statement not covered" >hashSha256Sync(Buffer.from(challengeText))</span>
&nbsp;
    const signatureBuffer = <span class="cstat-no" title="statement not covered" >signer.sign(digest)</span>
    const signatureWithHash = <span class="cstat-no" title="statement not covered" >script.signature.encode(</span>
...
&nbsp;
    ...
    const signature = <span class="cstat-no" title="statement not covered" >signatureWithHash.toString('hex').slice(0, -2)</span>
&nbsp;
    const publickey = <span class="cstat-no" title="statement not covered" >getPublicKeyFromPrivate(signerKeyHex)</span>
    const token = <span class="cstat-no" title="statement not covered" >Buffer.from(JSON.stringify(</span>
      { publickey, signature }
    )).toString('base64')
<span class="cstat-no" title="statement not covered" >    return token</span>
  } else {
<span class="cstat-no" title="statement not covered" >    throw new Error('Failed to connect to legacy gaia hub. If you operate this hub, please update.')</span>
  }
}
&nbsp;
/**
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!handlesV1Auth) {
<span class="cstat-no" title="statement not covered" >    return makeLegacyAuthToken(challengeText, signerKeyHex)</span>
  }
&nbsp;
...
&nbsp;
  ...
  const address = ecPairToAddress(hexStringToECPair(challengeSignerHex
                                    + (challengeSignerHex.length === 64 ? '01' : <span class="branch-1 cbranch-no" title="branch not covered" >'')</span>))
  return {
    url_prefix: readURL,
