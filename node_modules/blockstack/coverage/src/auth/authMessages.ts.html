...
<html lang="en">
<head>
    <title>Code coverage report for src/auth/authMessages.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">src/auth</a> authMessages.ts
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">83.87% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>52/62</span>
      </div>
      <div class='fl pad1y space-right2'>
...
<span class="cline-any cline-yes">8x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">// eslint-disable-next-line import/no-unassigned-import
 ...
 * @ignore
 */
export function <span class="fstat-no" title="function not covered" >generateTransitKey(</span>) {
  const transitKey = <span class="cstat-no" title="statement not covered" >makeECPrivateKey()</span>
...
export function makeAuthRequest(
  ...
  redirectURI?: string, 
  manifestURI?: string, 
  scopes: Array&lt;AuthScope | string&gt; = <span class="branch-0 cbranch-no" title="branch not covered" >DEFAULT_SCOPE.slice(),</span>
  appDomain?: string,
  expiresAt: number = <span class="branch-0 cbranch-no" title="branch not covered" >nextMonth().getTime(),</span>
  extraParams: any = <span class="branch-0 cbranch-no" title="branch not covered" >{}</span>
): string {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!transitPrivateKey) {
<span class="cstat-no" title="statement not covered" >    transitPrivateKey = new UserSession().generateAndStoreTransitKey()</span>
  }
&nbsp;
  const getWindowOrigin = <span class="fstat-no" title="function not covered" >(p</span>aramName: string) =&gt; {
    const location = <span class="cstat-no" title="statement not covered" >getGlobalObject('location', { </span>
      throwIfUnavailable: true, 
      usageDesc: `makeAuthRequest([${paramName}=undefined])` 
    })
<span class="cstat-no" title="statement not covered" >    return location.origin</span>
  }
  
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!redirectURI) {
<span class="cstat-no" title="statement not covered" >    redirectURI = `${getWindowOrigin('redirectURI')}/`</span>
  }
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!manifestURI) {
<span class="cstat-no" title="statement not covered" >    manifestURI = `${getWindowOrigin('manifestURI')}/manifest.json`</span>
  }
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!appDomain) {
<span class="cstat-no" title="statement not covered" >    appDomain = getWindowOrigin('appDomain')</span>
  }
&nbsp;
  /* Create the payload */
...
export async function decryptPrivateKey(privateKey: string,
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof decrypted !== 'string') {
<span class="cstat-no" title="statement not covered" >    throw new Error('Unable to correctly decrypt private key')</span>
  } else {
    return decrypted
  }
}
...
export async function makeAuthResponse(privateKey: string,
                                       profile: {} = <span class="branch-0 cbranch-no" title="branch not covered" >{},</span>
                                       username: string = <span class="branch-0 cbranch-no" title="branch not covered" >null,</span>
...
&nbsp;
  ...
  if (appPrivateKey !== undefined &amp;&amp; appPrivateKey !== null) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (transitPublicKey !== undefined &amp;&amp; transitPublicKey !== null) {
      privateKeyPayload = await encryptPrivateKey(transitPublicKey, appPrivateKey)
      <span class="missing-if-branch" title="if path not taken" >I</span>if (coreToken !== undefined &amp;&amp; coreToken !== null) {
<span class="cstat-no" title="statement not covered" >        coreTokenPayload = await encryptPrivateKey(transitPublicKey, coreToken)</span>
      }
    }
    additionalProperties = {
      email: metadata.email ? <span class="branch-0 cbranch-no" title="branch not covered" >metadata.email </span>: null,
      profile_url: metadata.profileUrl ? <span class="branch-0 cbranch-no" title="branch not covered" >metadata.profileUrl </span>: null,
      hubUrl,
      blockstackAPIUrl,
      associationToken,
      version: VERSION
