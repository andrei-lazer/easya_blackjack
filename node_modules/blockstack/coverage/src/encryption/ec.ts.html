...
<html lang="en">
<head>
    <title>Code coverage report for src/encryption/ec.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">src/encryption</a> ec.ts
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">89.76% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>114/127</span>
      </div>
      <div class='fl pad1y space-right2'>
...
<span class="cline-any cline-yes">12x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { ec as EllipticCurve } from 'elliptic'
...
function equalConstTime(b1: Buffer, b2: Buffer) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (b1.length !== b2.length) {
...
export function getHexFromBN(bnInput: BN): string {
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (hexOut.length === 64) {
    return hexOut
  } else <span class="cstat-no" title="statement not covered" >if (hexOut.length &lt; 64) {</span>
    // pad with leading zeros
    // the padStart function would require node 9
    const padding = <span class="cstat-no" title="statement not covered" >'0'.repeat(64 - hexOut.length)</span>
<span class="cstat-no" title="statement not covered" >    return `${padding}${hexOut}`</span>
  } else {
<span class="cstat-no" title="statement not covered" >    throw new Error('Generated a &gt; 32-byte BN for encryption. Failing.')</span>
  }
}
&nbsp;
/**
...
export function getBufferFromBN(bnInput: BN): Buffer {
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (result.byteLength !== 32) {
<span class="cstat-no" title="statement not covered" >    throw new Error('Generated a 32-byte BN for encryption. Failing.')</span>
  }
  return result
}
...
&nbsp;
  ...
  } else <span class="missing-if-branch" title="else path not taken" >E</span>if (opts.cipherTextEncoding === 'base64') {
    encodedCipherTextLength = getBase64OutputLength(cipherTextLength)
  } else {
<span class="cstat-no" title="statement not covered" >    throw new Error(`Unexpected cipherTextEncoding "${opts.cipherTextEncoding}"`)</span>
  }
...
&nbsp;
  ...
  } else <span class="missing-if-branch" title="else path not taken" >E</span>if (cipherTextEncoding === 'base64') {
    cipherTextString = cipherText.toString('base64')
  } else {
<span class="cstat-no" title="statement not covered" >    throw new Error(`Unexpected cipherTextEncoding "${cipherTextEncoding}"`)</span>
  }
...
<span class="cstat-no" title="statement not covered" >    throw new FailedDecryptionError('Unable to get public key from cipher object. '</span>
...
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (!cipherObject.cipherTextEncoding || <span class="branch-1 cbranch-no" title="branch not covered" >cipherObject.cipherTextEncoding === 'hex')</span> {
    cipherTextBuffer = Buffer.from(cipherObject.cipherText, 'hex')
  } else <span class="cstat-no" title="statement not covered" >if (cipherObject.cipherTextEncoding === 'base64') {</span>
<span class="cstat-no" title="statement not covered" >    cipherTextBuffer = Buffer.from(cipherObject.cipherText, 'base64')</span>
  } else {
<span class="cstat-no" title="statement not covered" >    throw new Error(`Unexpected cipherTextEncoding "${cipherObject.cipherText}"`)</span>
  }
&nbsp;
...
function getBuffer(content: string | ArrayBuffer | Buffer) {
  ...
  else <span class="missing-if-branch" title="if path not taken" >I</span>if (content instanceof ArrayBuffer) <span class="cstat-no" title="statement not covered" >return Buffer.from(content)</span>
